...
against file names, and applying them in the order they were provided.\nSee ",
    ...
    fn parse_files(mut self, aliases: &[&str]) -> Self {
        match self.raw.args_for(aliases) {
            None => exit::abort(ExitCode::MissingRequiredCliArgument(format!(
                "{:?}",
                aliases
            ))),
            Some(args) => {
    ...
    fn validate_patterns(raw_patterns: &[String], aliases: &[&str]) {
        ...
        if len < 2 {
            exit::abort(ExitCode::NotEnoughPatterns(format!("{:?}", &patterns)));
        }
        if len % 2 != 0 {
            exit::abort(ExitCode::MalformedPattern(format!("{:?}", &patterns)));
        }
    }
        ...
        match self.raw.args_for(&aliases) {
            ...
            Some(patterns) => {
                ...
                while idx < patterns.len() {
                    ...
                    let result = Regex::new(&patterns[idx]).case_insensitive().named("regex");
                    match result {
                        Ok(regex) => {
